...
async function resolveItemCareDisplayName(req: HookReq, itemId: number) {
...
export const populateItemCareDisplayNameHook: CollectionBeforeValidateHook = async ({
  data,
  operation,
  originalDoc,
  req,
  ...
  return {
    ...data,
    displayName: await resolveItemCareDisplayName(req, itemId),
  }
}

export const hydrateItemCareDisplayNameHook: CollectionAfterReadHook = async ({ doc, req }) => {
